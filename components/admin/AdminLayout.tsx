...
function SidebarContent({ user, handleSignOut }: { user: any, handleSignOut: () => void }) {
  return (
    <div className="flex flex-col h-full bg-stone-800 text-white">
      {/* Logo */}
      <div className="flex items-center px-6 py-6 border-b border-stone-700">
        <Link href="/admin" className="flex items-center">
          ...
          <div>
            <h1 className="text-xl font-serif font-bold text-white">LuxeCustomized</h1>
    ...
    <div className="min-h-screen bg-stone-900">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-stone-800 border-b border-stone-700 h-16 flex items-center px-4">
        <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
          ...
          <SheetContent side="left" className="p-0 bg-stone-800 border-r-0 w-80">
            <SidebarContent user={user} handleSignOut={handleSignOut} />
          </SheetContent>
        </Sheet>
        <div className="ml-4 text-white font-bold">Admin</div>
      </header>
